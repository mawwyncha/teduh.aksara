...
export const PermissionModal: React.FC<PermissionModalProps> = ({ type, onAccept, onDeny }) => {
  ...
  const iconBg = isMic 
    ...
    : isFactCheck
      ? (theme === 'flower' ? 'bg-indigo-100 text-indigo-500' : 'bg-indigo-50 dark:bg-indigo-900/20 text-indigo-400')
  ...
  const getTitle = () => {
    if (isMic) return 'Izinkan Tara Mendengar?';
    if (isFactCheck) return 'Izinkan Tara Menelusuri Rujukan?';
    return 'Izinkan Tara Menelusuri Web?';
  };

  const getDescription = () => {
    if (isMic) return 'Tara perlu meminjam pendengaranmu untuk mengubah suara menjadi aksara. Rekaman suaramu hanya diproses seketika untuk transkripsi dan tidak akan pernah kami simpan di dahan ingatan permanen kami.';
    if (isFactCheck) return 'Tara akan menyisir dahan-dahanan rujukan digital di internet untuk memverifikasi kebenaran setiap pernyataan dalam naskahmu. Proses ini memerlukan koneksi ke layanan pencarian eksternal untuk memberikan hasil yang akurat.';
    return 'Naskahmu akan dibandingkan dengan berbagai dahan teks di web luas untuk memastikan kemurniannya. Kami menjamin privasi naskahmu; pencarian ini hanya untuk verifikasi keaslian tanpa menyimpan naskahmu di tempat lain.';
  };
    ...
    <div className="fixed inset-0 z-[200] flex items-center justify-center p-6 bg-black/60 backdrop-blur-md animate-in fade-in duration-300">
      <div className={`${modalBg} w-full max-w-md rounded-[2.5rem] p-8 sm:p-10 shadow-2xl text-center border ${borderColor}`}>
        <div className={`w-24 h-24 mx-auto mb-8 rounded-full flex items-center justify-center shadow-inner ${iconBg}`}>
          {isMic ? (
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 1a3 3 0 0 0-3 3v8a3 3 0 0 0 6 0V4a3 3 0 0 0-3-3z"/><path d="M19 10v2a7 7 0 0 1-14 0v-2"/><line x1="12" cy="19" x2="12" y2="23"/><line x1="8" y1="23" x2="16" y2="23"/></svg>
          ) : isFactCheck ? (
            <svg width="40" height="40" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><path d="M12 22s8-4 8-10V5l-8-3-8 3v7c0 6 8 10 8 10z"/><path d="M9 12l2 2 4-4"/></svg>
          ) : (
